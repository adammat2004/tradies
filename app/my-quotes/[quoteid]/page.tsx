...
const Page = () => {
  ...
  const bodyContent = useMemo(() => {
    ...
    switch (step) {
      ...
      case Steps.Temp2:
        return (
          <div ref={pdfRef} className="bg-white rounded-xl border border-gray-200 p-6 md:p-10 shadow-md space-y-8 md:space-y-10">
            {/* Company and Quote Info */}
            <div className="flex flex-col md:flex-row justify-between gap-4 md:gap-0">
              <div>
                <h2 className="text-xl md:text-2xl font-bold text-gray-800">{quote.companyName}</h2>
            ...
            {/* Customer and Company Contact Info */}
                    ...
                    <thead className="bg-gray-100">
                      <tr>
                        {table.columns.map((col) => (
                          <th key={col} className="px-3 py-2 border border-gray-300 text-left whitespace-nowrap">{col}</th>
                        ))}
                        <th className="px-3 py-2 border border-gray-300 text-left whitespace-nowrap">Total</th>
                      </tr>
                    </thead>
                    <tbody>
                      {table.items.map((item) => (
                        <tr key={item.id} className="border-t border-gray-200">
                          {table.columns.map((col) => (
                            <td key={col} className="px-3 py-2 border border-gray-200 whitespace-nowrap">{item.data[col] || ''}</td>
                          ))}
                          {/*<td className="px-3 py-2 border border-gray-200 whitespace-nowrap">
                            {item.total
                            : ''}
                            </td>*/}
                            <td className="px-3 py-2 border border-gray-200 whitespace-nowrap" key={item.data['total']}>{item.data['total'] || ''}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
    ...
    <div className="max-w-5xl mx-auto px-4 py-8 font-sans">
      ...
      <div className="flex gap-4 justify-end mb-4 pt-6">
        ...
        <button
          ...
          className="hidden md:block md:bg-blue-600 md:text-white md:px-4 md:py-2 md:rounded-lg md:hover:bg-blue-700 md:shadow-md"
        >
          Save as PDF
        </button>
        <button
