 ...
 * - Lightweight page-level lightbox for images
 */
...
const currency = (n?: number | null, code = "EUR") =>
  typeof n === "number" ? new Intl.NumberFormat(undefined, { style: "currency", currency: code }).format(n) : undefined;
...
function MyRequestCard({
  ...
  const confirmedWindow = req.confirmedWindowId ? req.windows.find((w) => w.id === req.confirmedWindowId) : null;
            ...
            <div className="rounded-xl border p-3">
              ...
              <div className="mt-1 flex items-center gap-2 font-semibold text-slate-900">
                <svg className="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                  <path d="M21 10c0 4.97-9 13-9 13S3 14.97 3 10a9 9 0 1 1 18 0Z" />
              ...
              <ul className="mt-2 space-y-2 text-sm">
                {req.windows.map((w) => (
                  <li key={w.id} className="flex items-center justify-between gap-2">
                    ...
                    <span className="inline-flex items-center gap-2 rounded-full border px-2 py-0.5 text-xs text-slate-700">
                      {duration(w.startAt, w.endAt)}
                    </span>
                  </li>
                ))}
              </ul>
              ...
              <div className="mt-2 grid grid-cols-2 gap-2 sm:grid-cols-3">
                {req.pictures.map((src, i) => (
                  <button
                    ...
                    className="aspect-video overflow-hidden rounded-lg border focus:outline-none focus:ring-2 focus:ring-slate-400"
                    aria-label={`Open picture ${i + 1} in viewer`}
                  >
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                    <img
                      src={src}
                      alt={`picture-${i}`}
                      className="h-full w-full object-cover transition-transform hover:scale-[1.02] cursor-zoom-in"
                    />
                  </button>
                ))}
              </div>
        ...
        <aside className="space-y-3">
          <div className="rounded-xl border p-3">
            ...
            <div className="mt-1 text-xs text-slate-500">
              Service: <span className="font-medium text-slate-800">{req.service?.name || req.serviceId || "—"}</span>
            </div>
            {req.status === "accepted" ? (
              <div className="mt-3 rounded-lg border border-emerald-200 bg-emerald-50 p-2 text-xs text-emerald-900">
                ...
                <div>{req.confirmedWindowId ? fmtRange(req.windows.find(w => w.id === req.confirmedWindowId)!.startAt, req.windows.find(w => w.id === req.confirmedWindowId)!.endAt) : "No time window confirmed"}</div>
              </div>
            ) : (
              <div className="mt-3 rounded-lg border border-sky-200 bg-sky-50 p-2 text-xs text-sky-900">Awaiting confirmation</div>
            )}
          </div>
          ...
          <div className="rounded-xl border p-3">
            ...
            <div className="mt-2 flex flex-wrap gap-2">
              <a
                href={`mailto:${req.listing?.email ?? ""}`}
                className="rounded-md border bg-white px-3 py-1.5 text-sm font-semibold text-slate-900 hover:bg-slate-50"
              >
                Email provider
              </a>
              <a
                href={`tel:${req.listing?.phone ?? ""}`}
                className="rounded-md border bg-white px-3 py-1.5 text-sm font-semibold text-slate-900 hover:bg-slate-50"
              >
                Call provider
              </a>
...
export default function MyRequestsPage({ userId }: RequestsPageProps) {
  ...
  const [tab, setTab] = useState<"all" | "pending" | "accepted" | "completed" | "declined">("all");
  ...
  // Load requests once we have a real userId
  useEffect(() => {
    if (!userId) return;
    ...
    (async () => {
      try {
        setLoading(true);
        const res = await fetch(`/api/customer/my-requests?userId=${encodeURIComponent(userId)}`, { cache: "no-store" });
  ...
  const addWindow = async (id: string, startAt: string, endAt: string) => {
    try {
      await fetch(`/api/customer/requests/${id}/windows`, {
        method: "POST",
      ...
      <header className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row sm:items-center">
          <div className="flex items-center gap-1 rounded-full border bg-white p-1">
            {(["all", "pending", "accepted", "completed", "declined"] as const).map((t) => (
              <button
                key={t}
                onClick={() => setTab(t)}
                className={`rounded-full px-3 py-1 text-sm font-semibold ${
                  tab === t ? "bg-slate-900 text-white" : "text-slate-900 hover:bg-slate-100"
                }`}
              >
